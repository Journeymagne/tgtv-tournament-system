...
function backSVG(data,group,landscape){
 ...
 return '<svg xmlns="http://www.w3.org/2000/svg" width="'+w+'" height="'+h+'" viewBox="0 0 '+w+' '+h+'"><rect width="'+w+'" height="'+h+'" fill="#141718"/><rect x="8" y="8" width="'+(w-16)+'" height="'+(h-16)+'" rx="8" fill="none" stroke="'+accent+'"/><path d="M12 '+(h*.64)+'H'+(w-12)+'" stroke="'+accent+'" stroke-width="3"/>'+title+'<text x="'+w/2+'" y="'+(h*.77)+'" text-anchor="middle" fill="'+accent+'" font-family="Display" font-size="16">'+esc(group.label)+'</text><text x="'+w/2+'" y="'+(h-22)+'" text-anchor="middle" fill="#9da69f" font-family="Roboto" font-size="6">KILL TEAM · '+esc(data.team.version||'CUSTOM TEAM')+'</text></svg>';
}
